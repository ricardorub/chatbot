...
def moderator_dashboard():
    ...
    feedback_over_time = db.session.query(func.date(Feedback.submitted_at), func.count(Feedback.id)).group_by(func.date(Feedback.submitted_at)).order_by(func.date(Feedback.submitted_at)).all()
    users_over_time = db.session.query(func.date(User.created_at), func.count(User.id)).group_by(func.date(User.created_at)).order_by(func.date(User.created_at)).all()

    timeline_labels = sorted(list(set([f[0].strftime('%Y-%m-%d') for f in feedback_over_time] + [u[0].strftime('%Y-%m-%d') for u in users_over_time])))
    feedback_data = [dict(feedback_over_time).get(l, 0) for l in timeline_labels]
    user_data = [dict(users_over_time).get(l, 0) for l in timeline_labels]
